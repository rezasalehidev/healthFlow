...
/**
 * Polls PENDING clinical outbox rows and publishes them to RabbitMQ (at-least-once).
 */
@Injectable()
export class ClinicalOutboxRelayService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(ClinicalOutboxRelayService.name);
  ...
  onModuleInit(): void {
    ...
    this.timer.unref?.();
    this.logger.log({ message: 'clinical outbox relay started', pollMs: this.pollMs });
  }
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      this.logger.error({
        message: 'clinical outbox relay tick failed',
  ...
  async publishPending(): Promise<number> {
    ...
    for (const row of rows) {
      try {
        ...
        this.logger.error({
          message: 'clinical outbox publish failed',
          eventId: row.eventId,
          attempts: nextAttempts,
          failed,
          error: message,
    ...
    if (published > 0) {
      this.logger.log({ message: 'clinical outbox published', count: published });
    }
    return published;
  }
}
